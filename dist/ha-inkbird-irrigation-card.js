...
function dt(t){return function(t){return(e,i)=>"object"==typeof i?lt(t,e,i):((t,e,i)=>{const s=e.hasOwnProperty(i);return e.constructor.createProperty(i,t),s?Object.getOwnPropertyDescriptor(e,i):void 0})(t,e,i)}({...t,state:!0,attribute:!1})}const pt=["#4CAF50","#2196F3","#FF9800","#9C27B0","#00BCD4","#F44336"];let _t=class extends rt{constructor(){super(...arguments),this._loading=new Set}static getConfigElement(){return document.createElement("ha-inkbird-irrigation-card-editor")}static getStubConfig(){return{type:"custom:ha-inkbird-irrigation-card",entity_prefix:"inkbird_iic_600"}}setConfig(t){this._config=t}set hass(t){this._hass=t,this.requestUpdate()}getCardSize(){return 4}get _prefix(){return this._config.entity_prefix||"inkbird_iic_600"}get _zones(){return this._config.zones||[1,2,3,4,5,6]}_zoneName(t){return this._config.zone_names?.[t]||`Zone ${t}`}_zoneColor(t){const e=this._zones.indexOf(t);return pt[e%pt.length]}_zoneSwitch(t){return this._hass?.states[`switch.${this._prefix}_zone_${t}`]}_zoneRemaining(t){const e=this._hass?.states[`sensor.${this._prefix}_zone_${t}_time_remaining`];return e&&parseInt(e.state)||0}_zoneElapsed(t){const e=this._hass?.states[`sensor.${this._prefix}_zone_${t}_time_elapsed`];return e&&parseInt(e.state)||0}_zoneDuration(t){const e=this._hass?.states[`number.${this._prefix}_zone_${t}_duration`];return e&&parseInt(e.state)||30}_zoneIsActive(t){return"on"===this._zoneSwitch(t)?.state}get _mode(){return this._hass?.states[`sensor.${this._prefix}_mode`]?.state||"auto"}get _skipSchedule(){return"on"===this._hass?.states[`switch.${this._prefix}_skip_schedule`]?.state}get _mainValve(){return"on"===this._hass?.states[`switch.${this._prefix}_main_valve`]?.state}get _rainSensor(){return"on"===this._hass?.states[`switch.${this._prefix}_rain_sensor`]?.state}get _activeZones(){return this._zones.filter(t=>this._zoneIsActive(t))}async _refreshEntity(){await new Promise(t=>setTimeout(t,500));for(const t of this._zones)await(this._hass?.callService("homeassistant","update_entity",{entity_id:`switch.${this._prefix}_zone_${t}`}))}async _toggleZone(t){const e=`zone_${t}`;this._loading=new Set([...this._loading,e]);try{const e=this._zoneIsActive(t),i=`switch.${this._prefix}_zone_${t}`;await(this._hass?.callService("switch",e?"turn_off":"turn_on",{entity_id:i})),await this._refreshEntity()}finally{this._loading=new Set([...this._loading].filter(t=>t!==e))}}async _startZone(t,e){const i=`zone_${t}`;this._loading=new Set([...this._loading,i]);try{await(this._hass?.callService("number","set_value",{entity_id:`number.${this._prefix}_zone_${t}_duration`,value:e})),await(this._hass?.callService("switch","turn_on",{entity_id:`switch.${this._prefix}_zone_${t}`})),await this._refreshEntity()}finally{this._loading=new Set([...this._loading].filter(t=>t!==i))}}async _stopAll(){this._loading=new Set([...this._loading,"stop_all"]);try{for(const t of this._zones)this._zoneIsActive(t)&&await(this._hass?.callService("switch","turn_off",{entity_id:`switch.${this._prefix}_zone_${t}`}));await this._refreshEntity()}finally{this._loading=new Set([...this._loading].filter(t=>"stop_all"!==t))}}async _toggleSwitch(t){const e=`sw_${t}`;this._loading=new Set([...this._loading,e]);try{const e="on"===this._hass?.states[t]?.state;await(this._hass?.callService("switch",e?"turn_off":"turn_on",{entity_id:t})),await new Promise(t=>setTimeout(t,1e3)),await(this._hass?.callService("homeassistant","update_entity",{entity_id:t})),await(this._hass?.callService("homeassistant","update_entity",{entity_id:`switch.${this._prefix}_main_valve`})),await(this._hass?.callService("homeassistant","update_entity",{entity_id:`switch.${this._prefix}_rain_sensor`})),await new Promise(t=>setTimeout(t,500))}finally{this._loading=new Set([...this._loading].filter(t=>t!==e))}}async _setDuration(t,e){await(this._hass?.callService("number","set_value",{entity_id:`number.${this._prefix}_zone_${t}_duration`,value:e}))}render(){if(!this._config||!this._hass)return B;const t=this._activeZones;return F`
      <ha-card>
        <div class="card-header">
          ...
          <div class="header-right">
            ${this._skipSchedule?F`<span class="badge badge--skip">Skipped</span>`:B}
    ...
    `}_renderSwitches(){const t=this._mainValve,e=this._rainSensor,i=this._skipSchedule;return F`
      <div class="switches-row">
        <button class="sw-btn ${t?"sw-btn--on":""} ${this._loading.has(`sw_switch.${this._prefix}_main_valve`)?"sw-btn--loading":""}" @click=${()=>this._toggleSwitch(`switch.${this._prefix}_main_valve`)}>
          ${this._loading.has(`sw_switch.${this._prefix}_main_valve`)?F`<ha-icon icon="mdi:loading" class="spin"></ha-icon>`:F`<ha-icon icon="mdi:valve"></ha-icon>`}
          <span>Valve</span>
        </button>
        <button class="sw-btn ${e?"sw-btn--on":""} ${this._loading.has(`sw_switch.${this._prefix}_rain_sensor`)?"sw-btn--loading":""}" @click=${()=>this._toggleSwitch(`switch.${this._prefix}_rain_sensor`)}>
          ${this._loading.has(`sw_switch.${this._prefix}_rain_sensor`)?F`<ha-icon icon="mdi:loading" class="spin"></ha-icon>`:F`<ha-icon icon="mdi:weather-rainy"></ha-icon>`}
          <span>Rain</span>
        </button>
        ${i?F`
    ...
    `}_renderZone(t){const e=this._zoneIsActive(t),i=this._zoneRemaining(t),s=this._zoneElapsed(t),n=this._zoneDuration(t),o=e&&s+i>0?s/(s+i)*100:0,r=this._zoneColor(t);return F`
        ...
        <div class="zone-main">
          ...
          ${e?F`
            <button class="zone-btn zone-btn--active" @click=${()=>this._toggleZone(t)}>
              ${this._loading.has(`zone_${t}`)?F`<ha-icon icon="mdi:loading" class="spin"></ha-icon>`:F`<ha-icon icon="mdi:stop"></ha-icon>`}
            </button>
          `:F`
            <div class="zone-controls">
              <select class="dur-select" @change=${e=>this._setDuration(t,parseInt(e.target.value))}>
                ${[5,10,15,20,30,45,60,90,120].map(t=>F`
              ...
              <button class="zone-start-btn" @click=${()=>this._startZone(t,n)} ?disabled=${this._loading.has(`zone_${t}`)}>
                ${this._loading.has(`zone_${t}`)?F`<ha-icon icon="mdi:loading" class="spin"></ha-icon>`:F`<ha-icon icon="mdi:water"></ha-icon>`}
              </button>
            </div>
          `}
        </div>
  ...
  `,t([dt()],_t.prototype,"_config",void 0),t([dt()],_t.prototype,"_loading",void 0),_t=t([ht("ha-inkbird-irrigation-card")],_t),window.customCards=window.customCards||[],window.customCards.push({type:"ha-inkbird-irrigation-card",name:"Inkbird Irrigation",description:"Manage your Inkbird IIC-600 irrigation controller: zone controls, progress, and status.",preview:!0});let ut=class extends rt{constructor(){super(...arguments),this._config={},this._schema=[{name:"title",selector:{text:{}},label:"Card Title"},{name:"entity_prefix",selector:{text:{}},label:"Entity Prefix (e.g. inkbird_iic_600)"},{name:"zone_1_entity",selector:{entity:{domain:"switch"}},label:"Zone 1 Switch"},{name:"zone_2_entity",selector:{entity:{domain:"switch"}},label:"Zone 2 Switch"},{name:"zone_3_entity",selector:{entity:{domain:"switch"}},label:"Zone 3 Switch"},{name:"zone_4_entity",selector:{entity:{domain:"switch"}},label:"Zone 4 Switch"},{name:"zone_5_entity",selector:{entity:{domain:"switch"}},label:"Zone 5 Switch"},{name:"zone_6_entity",selector:{entity:{domain:"switch"}},label:"Zone 6 Switch"}]}set hass(t){this._hass=t}setConfig(t){this._config=t}render(){return F`
      <ha-form
        .hass=${this._hass}
        .data=${this._config}
        .schema=${this._schema}
      ...
      ></ha-form>
    `}_handleFormChanged(t){const e={...this._config,...t.detail.value},i=[];for(let t=1;t<=6;t++)e[`zone_${t}_entity`]&&i.push(t);i.length>0&&(e.zones=i),this._config=e,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e}}))}};t([dt()],ut.prototype,"_config",void 0),ut=t([ht("ha-inkbird-irrigation-card-editor")],ut);export{_t as HaInkbirdIrrigationCard,ut as HaInkbirdIrrigationCardEditor};
